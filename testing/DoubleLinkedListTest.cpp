...
TYPED_TEST(DoubleLinkedListTest, ThrowsOnEmpty) {
  ...
  ASSERT_THROW(const auto& _ = dll.back(), DoublyLinkedListEmptyError);
}

TYPED_TEST(DoubleLinkedListTest, ThrowsAtEnd) {
  DoublyLinkedList<TypeParam> dll(TestFixture::constructorArgs);
  ASSERT_THROW(const auto& _ = *dll.end(), DoublyLinkedListOutOfBoundsError);
  ASSERT_THROW(const auto& _ = *dll.rend(), DoublyLinkedListOutOfBoundsError);
  ASSERT_THROW(const auto& _ = *dll.crend(), DoublyLinkedListOutOfBoundsError);
}
...
TYPED_TEST(DoubleLinkedListTest, ThrowsBeforeBeginning) {
  ...
  --fitr;
  --ritr;
  ASSERT_THROW(const auto& _ = *fitr, DoublyLinkedListOutOfBoundsError);
  ASSERT_THROW(const auto& _ = *ritr, DoublyLinkedListOutOfBoundsError);
}
